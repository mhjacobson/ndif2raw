...
static int find_resource(const AppleSingleFile *as_file, const ResType type, const ResID id,
    ...
    for (uint16_t i = 0; i < type_count; i++) {
        ...
        uint32_t type_entry_offset = fork_base + header.map_offset +
                                     type_list_offset + 2 + (i * 8);
        ...
        if (type_entry.type == type) {
            ...
            uint32_t ref_list_offset = fork_base + header.map_offset +
            ...
            for (uint16_t j = 0; j < type_entry.count; j++) {
                ...
                if (read_at_offset(as_file->file, ref_buffer,
                                   ref_list_offset + (j * 12), sizeof ref_buffer)) {
                    return 1;
                }
                ...
                if (ref.id == id) {
                    ...
                    const uint32_t res_data_offset = fork_base + header.data_offset + ref.data_offset;
                    ...
                    *offset = res_data_offset + 4; // Skip size bytes
                    *size = res_size;
...
static void print_resource_info(const AppleSingleFile *const as_file) {
    ...
    // Read map header
    const uint32_t map_base = fork_base + header.map_offset;
    uint8_t map_header[28];
    ...
    for (uint16_t i = 0; i < type_count; i++) {
        ...
        const uint32_t type_entry_offset = map_base + type_list_offset + 2 + (i * 8);
        ...
        const uint32_t ref_list_offset = map_base + type_list_offset + type_offset;
